...
ENV PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1 DJANGO_SETTINGS_MODULE=ce.settings

# Add metadata
LABEL org.opencontainers.image.source=https://github.com/veselosky/control-escape

# Use Granian to run the application
EXPOSE 8000
CMD ["granian", "--interface", "wsgi", "--workers", "4", "--host", "0.0.0.0", "ce.wsgi:application"]
